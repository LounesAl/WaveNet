    ...
    def __init__(self, batch_size=16, set_name='train'):
        ...
        with open(_data_path + 'preprocess/meta/%s.csv' % set_name) as csv_file:
            ...
            for row in reader:
                # mfcc file
                #print(row[0])
                mfcc_file.append(_data_path + 'preprocess/mfcc/' + "p225_001_mic1.flac" + '.npy')
                
                # label info ( convert to string object for variable-length support )
                label.append(np.asarray(row[1:], dtype=np.int).tostring())
               
        #mfcc_file='asset/data/preprocess/mfcc/p225_001_mic1.flac.npy'
        ...
        #label_q, mfcc_file_q= tf.data.Dataset([label_t, mfcc_file_t])
        label_q, mfcc_file_q=tf.compat.v1.train.slice_input_producer([label_t, mfcc_file_t], shuffle=True, capacity=256)
        print(label_q)
        
        #create label, mfcc queue
      #tf.sg_intx, tf.sg_floatx
        label_q, mfcc_q = _load_mfcc(source=[label_q, mfcc_file_q],
                                     dtypes=[tf.int32, tf.float32],
                                     capacity=256, num_threads=64)

        # create batch queue with dynamic pad
        batch_queue = tf.compat.v1.train.batch([label_q, mfcc_q], batch_size,
                                     shapes=[(None,), (20, None)],
